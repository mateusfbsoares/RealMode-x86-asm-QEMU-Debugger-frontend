...
define print_regs
  ...
  printf "CX: %04X DX: %04X", $rcx, $rdx
  printf "SI: %04X DI: %04X ", $rsi, $rdi
  #printf "SP: %04X BP: %04X\n", $rsp, $rbp
  printf "SP: %04X BP: %04X", $rsp, $rbp
  ...
  printf "IP: %04X EIP:%08X", ((unsigned short)$eip & 0xFFFF), $eip
  #printf "CS:IP: %04X:%04X (0x%05X)\n", $rcs, ((unsigned short)$eip & 0xFFFF), $rip
  printf "CS:IP: %04X:%04X (0x%05X)", $rcs, ((unsigned short)$eip & 0xFFFF), $rip
  #printf "SS:SP: %04X:%04X (0x%05X)\n", $rss, $rsp, $r_ss_sp
  printf "SS:SP: %04X:%04X (0x%05X)", $rss, $rsp, $r_ss_sp
  #printf "SS:BP: %04X:%04X (0x%05X)\n", $rss, $rbp, $r_ss_bp
  printf "SS:BP: %04X:%04X (0x%05X)", $rss, $rbp, $r_ss_bp
end
document print_regs
Print CPU registers
end
